...
#include <awm/type_support_for_loaned_buffer.h>
...
public:
    ...
    void init(const char* topic) {
        ...
        awm::TypeSupportBase* ts = new awm::TypeSupportForLoanedBuffer("std_msgs/String#992ce8a1687cec8c8bd883ec73ca41d1");
...
void HelloWorldSubscriber::SubListener::on_data_available(dds::DataReader* reader) {
    ...
    // }
    dds::LoanableSequence<void*> data;
    dds::SampleInfoSeq infos;
    if (reader->take(data, infos, 1) == ReturnCode_t::RETCODE_OK) {
        awm::Buffer* sample = static_cast<awm::Buffer*>(data.buffer()[0]);
        std::cout << "length:" << sample->length << std::endl;
        for (int i = 0; i < sample->length; i++) {
            std::cout << int(sample->data[i]) << " ";
        } std::cout << std::endl;
